...
const Single = ({
  ...
  <Grid container spacing={2}>
    ...
    <Grid item xs={12} sm>
      ...
      {episode && !Array.isArray(episode) ? <Simple label="episode" value={episode} /> : null}

      {origin ? (
        <Link label="origin" url={'/location/' + findId(origin.url)} value={origin.name} />
      ) : null}
      {location ? (
        <Link label="location" url={'/location/' + findId(location.url)} value={location.name} />
      ) : null}
      ...
      {characters ? <MyArray label="characters" url="/character/" array={characters} /> : null}
    </Grid>
  </Grid>
);
